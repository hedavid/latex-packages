...
\begin{center}
	...
	\\
	\texttt{Merci à Denis Bitouzé et rafa4mat pour leurs retours !}
\end{tabular}
\end{center}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \MontreCode{Opacite} pour l'opacité du fond (\MontreCode{1} par défaut) ;
...
$\SurlignerFormule[Fond=pink,Bord=red,Opacite=0.25]{f(x)=\displaystyle\frac{1}{1+x}}<thick>$.\\
...
$\HighlightFormula[bg=pink,border=red,opacity=0.25]{f(x)=\displaystyle\frac{1}{1+x}}<thick>$.\\
