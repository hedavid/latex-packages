...
\verb|v0.42f|:~~~~Nouvelle boîte type \textit{feuille perforée}

\verb|v0.42e|:~~~~Correction d'un \textit{bug} avec \textsf{fontawesome}

\verb|v0.42d|:~~~~Améliorations du code + Clavier français

\verb|v0.42c|:~~~~Nouvelles icones (\textsf{solarbroken}) + nouveaux pictogrammes \textit{maths}

\verb|v0.42b|:~~~~Nouvelles icones (\textsf{badge AI}) + nouveaux pictogrammes \textit{maths}

\verb|v0.42a|~:~~~BoxNumber + sous-package \textsf{customenvs-mathpictos}

\verb|v0.41f|~:~~~Améliorations + nouveaux pictogrammes

\verb|v0.41e|~:~~~Bicolortitle + tcboxboard

\verb|v0.41d|~:~~~Sous-package \textsf{customenvs-macros}
...
\begin{itemize}
	...
	\item \MontreCode{enumitem}, \MontreCode{multicol}, \MontreCode{tabularray}, \MontreCode{xcolor} ;
...
\begin{itemize}
	...
	\item \MontreCode{$\mathtt{\langle}$nontikzbabl$\mathtt{\rangle}$} ;
	\item \MontreCode{$\mathtt{\langle}$noninlinegraphicx$\mathtt{\rangle}$} ;
	\item \MontreCode{$\mathtt{\langle}$nonfa$\mathtt{\rangle}$} /  \MontreCode{$\mathtt{\langle}$fa6$\mathtt{\rangle}$} / \MontreCode{$\mathtt{\langle}$fa7$\mathtt{\rangle}$}.
\end{itemize}
...
\subsection{Sous-package customenvs-tikzpictos (v0.20b)}
...
%type := wifi/network/stars/speedo/bullseye/skills/pill/calendar/clippy/dball
...
\subsection{Sous-package customenvs-exams (v0.1.5)}
...
L'idée est de pouvoir charger ces fonctionnalités de manière indépendante à \MontreCode{customenvs}.

\begin{DemoCode}{listing only}
%réponses à un QCM (cf section suivante si besoin, les clés sont identiques)
\ExamReponsesQCM%
  [%
...
\subsection{Sous-package customenvs-icons (v0.1.5)}

\MontreCode{customenvs} charge, pour \textit{des} petites icônes, le package \MontreCode{customenvs-icons}, qui est fourni avec.

L'idée est de pouvoir charger les petits icônes de manière indépendante à \MontreCode{customenvs}.

\begin{DemoCode}{listing only}
\ceicon%
  [%
    educ=TF,                        %booléen style 'educ'
    design=TF                       %booléen style 'design'
    height=...,                     %(d)auto / height / height+depth
    (d)strut=...                    %choix des boîtes (calculs de dimensions)
  ]%
...
Il existe également des icônes type \textit{honnêteté IA}, créés grâce à un travail d'Arnaud Durand.

(Lien : \texttt{\url{https://mathix.org/linux/archives/21431}}).
...
\subsection{Sous-package customenvs-mathpictos (v0.1c)}

\MontreCode{customenvs} charge, pour \textit{des} petites icônes, le package \MontreCode{customenvs-mathpictos}, qui est fourni avec.

L'idée est de pouvoir charger les petits icônes (réalisées en \TikZ) de manière indépendante à \MontreCode{customenvs}.

\begin{DemoCode}{listing only}
\cemaththemeicon%
  [includegraphics options]%
  {bordures=...,fond=TF,couleur fond=...}%
  {type}
\end{DemoCode}

\begin{DemoCode}{}
\cemaththemeicon[height=2cm]{bordures=11}{binomiale}%
\cemaththemeicon[height=2cm]{bordures=01,fond}{recurrence}
\end{DemoCode}

\subsection{Sous-package customenvs-macros (v0.1.0)}

\MontreCode{customenvs} charge, pour \textit{des} macros diverses et variées, le package \MontreCode{customenvs-macros}, qui est fourni avec.

\smallskip

IL s'agit de macros pour :

\begin{itemize}
	\item insérer une image avec remplissage vertical ;
	\item travailler sur des longueurs ;
	\item découper un argument en deux parties.
\end{itemize}
...
\begin{DemoCode}{}
% la case à cocher est \def\ReponsesQCMbox{\raisebox{-0.2ex}{\faSquare[regular]}}
...
\begin{itemize}
	...
	\textit{(il faut que le nombre d'éléments de cette clé soit en adéquation avec le nombre de niveaux !)}
...
\begin{itemize}
	\item \MontreCode{height} : hauteur ; \MontreCode{auto} par défaut
	\item \MontreCode{width} : largeur ; \MontreCode{5cm} par défaut
	\item \MontreCode{bgcolor} : couleur de fond ; \MontreCode{lightgray!50} par défaut
	\item \MontreCode{receivecolor} : couleur de réception ; \MontreCode{greenwa!66!white} par défaut
	\item \MontreCode{sendcolor} : couleur d'envoi ; \MontreCode{white} par défaut
	\item \MontreCode{txtwrite} : texte zone saisie ; \MontreCode{Message...} par défaut
	\item \MontreCode{fonttxt} : police ; \MontreCode{sffamily} par défaut
	\item \MontreCode{avatar} : avatar ; \MontreCode{\textbackslash faAddressCard} par défaut
	\item \MontreCode{showavatar} : booléen pour afficher l'avatar ; \MontreCode{false} par défaut
	\item \MontreCode{bw} : booléen pour passer en noir et blanc ; \MontreCode{false} par défaut
	\item \MontreCode{txtwidth} : largeur des bulles ; \MontreCode{0.55} par défaut.
\end{itemize}
...
\begin{EnvChatWA}{C. Pierquet}
	...
	\WaDate{aujourd'hui}
	\WaSend*{08:33}{Salut !!}
	\WaSend*{08:35}{Je vois que t'as WhApp du coup :-)}
	\WaRec{10:15}{Bah oui !!}
	\WaRec{10:18}{Tu pourrais me donner un coup de main ??}
	\WaRec{10:18}{J'ai du mal à gérer les tableaux en \LaTeX\ldots}
\end{EnvChatWA}
\end{DemoCode}
...
%exo1 alt, environnement par défaut
\begin{EnvtExoAlt}
Ceci est un exercice\ldots
\end{EnvtExoAlt}
\end{DemoCode}
...
\begin{itemize}
	...
	\end{itemize}
	\item le compteur (nommé \MontreCode{numeroexo} ou \MontreCode{numeroexoalt}) peut être désactivé et \textit{adpaté} via les commandes classiques de compteurs.
\end{itemize}
...
\subsection{'Petites' boîtes}

\subsubsection{Numéros encerclés}
...
\subsubsection{Boîte arrondie, petite boîte type marker}
...
\subsubsection{Numéro type 'boîte'}

\begin{DemoCode}{listing only}
\tkzboxnumber[clés]{numéro}
...
\begin{DemoCode}{}
%style global (pour visualiser l'influence de width auto)
...
\tkzboxnumber[bg color=red]{2}~\lipsum[1][1]\par
\tkzboxnumber[num color=yellow,shadow=false]{4}~\lipsum[1][1]\par
$\ldots$\par
...
\subsection{Boîtes de 'titres'}

\subsubsection{Titres de sujets d'examens, titres d'exercices}
...
\subsubsection{Bannière de titre}
...
\subsubsection{Boîte avec ornements}
...
\subsubsection{Titre 'bicolore'}

\begin{DemoCode}{listing only}
%inspiré d'un document de Joan Riguet
\tkzbicolortitle[clés]<options tikz>{titre ou ligne1§ligne2}
...
\begin{DemoCode}{}
\tkzbicolortitle%
  [left upper=CH-01,left lower=Fiche 1]%
  {TEST}
\end{DemoCode}
...
\tkzbicolortitle%
  [left upper=CH-01,left lower=Fiche 1]%
  {une semaine pour travailler§mes automatismes}
\end{DemoCode}

\begin{DemoCode}{}
\tkzbicolortitle%
  [main color=red,alt color=yellow,left upper=CH-01,
  left lower=Fiche 1,inner sep=1.5cm,left part=2cm,height=0.75cm]%
  {une semaine pour travailler§mes automatismes}
\end{DemoCode}
...
\subsection{Boîtes diverses}

\subsubsection{Boîte type message de forum}
...
%left           = booléen pour passer en mode avatar à droite
%swap-title    = booléen pour échanger les lignes du bloc avatar
...
\subsubsection{Boîte type 'tableau blanc'}

\begin{DemoCode}{listing only}
%inspiré d'un document de Joan Riguet
\begin{tcboxwhiteboard}[clés]{options tcbox}
%contenu
\end{tcboxwhiteboard}
...
\begin{DemoCode}{}
\begin{tcboxwhiteboard}%
  [border color=teal!25,alt color=red,width=\linewidth,bottom=false]%
  {height=5cm}
\lipsum[1][1-3]
\end{tcboxwhiteboard}
...
\subsubsection{Boîte type 'feuille perforée'}

\begin{DemoCode}{listing only}
%boîte
\begin{tcboxnotebook}[clés]{options tcbox}
%contenu
\end{tcboxnotebook}

%---Les clés disponibles sont :
% orientation = orientation de la boîte (V)
% bar         = affichage de la barre, en mode V (false)
% bg          = couleur de fond (white)
\end{DemoCode}

\begin{DemoCode}{listing only}
%formatage du titre
\NewDocumentCommand\tcboxnotebooktitle{ O{blue!50!black} m }{%
  \hspace{2.5mm}%
  \textcolor{#1}{%
    \bfseries\large\sffamily{\vphantom{(qÉ)}#2}%
  }\par\smallskip
}
\end{DemoCode}

\begin{DemoCode}{}
\begin{tcboxnotebook}[orientation=H]{width=10cm}
\tcboxnotebooktitle{Rappels utiles}
\lipsum[1]
\end{tcboxnotebook}
\end{DemoCode}

\begin{DemoCode}{}
\begin{tcboxnotebook}[bg=violet!25]{}
\tcboxnotebooktitle{Rappels utiles}
\lipsum[1]
\end{tcboxnotebook}
\end{DemoCode}
...
\section{Clavier français}

\subsection{Introduction}

L'idée est de proposer une commande pour afficher un clavier (type \texttt{fr 15'}), avec éventuellement des touches mises en valeur.

\begin{DemoCode}{listing only}
\ClavierFrancais[clés]<options tikzpicture{touches}
\end{DemoCode}
...
\subsection{Liste des touches}

Les touches disponibles sont :

\begin{ttfamily}
echap,F1,F2,F3,F4,F5,F6,F7,F8,F9,F10,F11,F12,printscreen,inser,suppr,NUMstar,NUMslash,off

square,N1,N2,N3,N4,N5,N6,N7,N8,N9,N0,rparen,equal,backspace,NUMplus,NUMminus,numlock

tab,A,Z,E,R,T,Y,U,I,O,P,circum,dollar,star,NUM7,NUM8,NUM9

capslock,Q,S,D,F,G,H,J,K,L,M,ugrave,enter,NUM4,NUM5,NUM6

leftmaj,geqleq,W,X,C,V,B,N,comma,semicolon,colon,excla,rightmaj,NUM1,NUM2,NUM3

leftctrl,fn,win,alt,space,altgr,rightctrl,NUM0,NUMdot,NUMenter,left,right,down,up
\end{ttfamily}

\subsection{Clés de personnalisations}

Les \MontreCode{clés} disponibles, à donner entre \MontreCode{[...]}, sont :

\begin{itemize}
	\item \MontreCode{Largeur=...} : largeur du clavier (\MontreCode{16cm} par défaut)
	\item \MontreCode{CouleurBordure=...} : bordure des touches
	\item \MontreCode{CouleurFond=...} : couleur de fond des touches
	\item \MontreCode{CouleurSelect=...} : couleur des touches sélectionnées
	\item \MontreCode{AfficheTouches=TF} : booléen pour afficher les étiquettes des touches
	\item \MontreCode{Complet=TF} : booléen pour afficher les étiquettes complètes.
\end{itemize}

\begin{DemoCode}{}
\ClavierFrancais[Complet]{}
\end{DemoCode}

\begin{DemoCode}{}
\ClavierFrancais{leftctrl,alt,inser}
\end{DemoCode}

\begin{DemoCode}{}
\ClavierFrancais[%
  Largeur=10cm,CouleurBordure=violet,%
  CouleurFond=orange!10,CouleurSelec=blue!50%
  ]{altgr,N4}
\end{DemoCode}

\begin{DemoCode}{}
\ClavierFrancais[Largeur=6cm]{}
\end{DemoCode}

\begin{DemoCode}{}
\ClavierFrancais[Largeur=4cm]<rotate=15>{}
\end{DemoCode}
...
\subsection{Insertion d'images avec remplissage de l'espace vertical (customenvs-macros)}
...
La clé \MontreCode{vCentrer} (parmi \MontreCode{hg/h/hd/g/m/d/bg/b/db/n} permet de spécifier un type de centrage H-V, si besoin, parmi \MontreCode{milieu/haut/bas/non} (\MontreCode{n} par défaut).

\begin{DemoCode}{listing only}
\InsererImage<vRemplir,vOffset=30mm,vCentrer=m>{example-image}
\end{DemoCode}
...
\subsection{Travailler sur des longueurs (customenvs-macros)}
...
Il est également possible de déterminer la hauteur (totale ou non) maximale de boîtes différentes.

\begin{DemoCode}{listing only}
%plus grande hauteur (sans profondeur pour la version étoilée)
\getheigheststring(*)[\macro]{liste d'éléments}
\end{DemoCode}

\NewTotalTCBox{\mytightbox}{ m }{size=tight,nobeforeafter,tcbox raise base}{#1}

\begin{DemoCode}{}
%boîtes individuelles
\storetotalheighttolength{\large\sffamily qB}{\malongueurA}\mytightbox{\large\sffamily qB} := \the\malongueurA

\storetotalheighttolength{\huge Éducation}{\malongueurB}\mytightbox{\huge Éducation} := \the\malongueurB

\storetotalheighttolength{$\tfrac{1}{x}$}{\malongueurC}\mytightbox{$\tfrac{1}{x}$} := \the\malongueurC

%macro globale
\getheigheststring[\maplusgrandelongueur]{\large\sffamily qB,\huge Éducation,$\tfrac{1}{x}$}%
\mytightbox{\large\sffamily qB}\mytightbox{\huge Éducation}\mytightbox{$\tfrac{1}{x}$} := \the\maplusgrandelongueur
\end{DemoCode}
...
\begin{DemoCode}{listing only}
%version étoilée avec affichage (en tikz) de la boîte support
\fittexttobox(*){texte}{largeur}{hauteur}
\end{DemoCode}

\begin{DemoCode}{}
%avec les boîtes
\fittexttobox*{TÉLÉPHONE}{2cm}{1cm}\\
\fittexttobox*{\bfseries\sffamily TÉLÉPHONE}{7cm}{1cm}\\
\fittexttobox*{TÉLÉPHONE}{3cm}{1cm}\\
\fittexttobox*{\ttfamily TÉLÉPHONE}{3cm}{1cm}\\
\fittexttobox*{TÉLÉPHONE}{2cm}{2cm}\\
\fittexttobox*{COMPLIMENTS}{10cm}{3.5cm}\\
\fittexttobox*{COMPLIMENTS}{14cm}{1.25cm}
\end{DemoCode}
