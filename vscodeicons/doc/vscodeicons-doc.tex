...
\title{The \textsf{vscodeicons} package (0.1.1) -- 2025/11/24}
\author{%
	Cédric Pierquet\\%
...
By default, icons are given with a small border, so the given height is the height of the \textit{full} box, not the height of the icons.
...
\vscodeicon[foldertype,height={1.2em/-0.4em}]{story}
\end{Verbatim}

{\Huge qb\vscodeicon[foldertype,height={1.2em/-0.4em}]{story}Xz}

\begin{Verbatim}[frame=single]
%fixed height
\vscodeicon[height=1in]{folder-opened}
\end{Verbatim}
...
\texttt{ifthen}, \texttt{calc}, \texttt{graphicx} and \texttt{simplekv} are loaded and used by the package.%, \texttt{xstring} 
