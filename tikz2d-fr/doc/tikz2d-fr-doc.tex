...
\newtcblisting{DemoCode}[1][]{%
	...
	listing options={%
		...
		classoffset=2,%
		alsoletter={-},%
		morekeywords={\TracerSegments,\DefinirPoints,\DefinirPoint,\MarquerPoint,\MarquerPoints,mainlevee,\ImagePoint,\ImagesPoints,mainleveealt},%
...
\verb|v0.1.3|~:~~~~Ajout d'un style alternatif pour la mainlevée

\verb|v0.1.2|~:~~~~Ajout d'une commande pour l'orthogone de Lill
...
L'idée est d'utiliser une déclaration légèrement différente du tracé à main levée, basée sur un code type \texttt{stylomainlevee}.

À noter que pour des chemins fermés ou des portions circulaires, le rendu peut ne pas être optimal.

\begin{DemoCode}[listing only]
	%la grille a été rajoutée pour la sortie
	\begin{tikzpicture}
		\draw[thick,mainleveealt] (0,0) rectangle (4,4) ;
	\end{tikzpicture}
	
	\begin{tikzpicture}
		\draw[thick,mainleveealt=1pt et 0.5pt] (0,0) rectangle (4,4) ;
	\end{tikzpicture}
	
	\begin{tikzpicture}
		\draw[thick,mainleveealt=1pt et 2pt]  (0,0) rectangle (4,4) ;
	\end{tikzpicture}
\end{DemoCode}
...
\section{Orthogone de Lill}

\input{OrthogoneLill.tex}
